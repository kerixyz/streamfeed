...
async function generateNewcSummaries(streamerName) {
  try {
    console.log(`Starting summary generation for streamer: ${streamerName}`);

    // Fetch messages for the streamer
    const result = await pool.query(
      "SELECT message FROM newc_messages WHERE streamer_name ILIKE $1",
      [streamerName]
    );
  ...
  module.exports = { generateSummaries, generateNewcSummaries };